...
#define bvalue(p)			((p)->_object._blob._bvalue)
#define blength(p)		((p)->_object._blob._length)
...
INTERFACE INLINE void *blob_value(pointer p) { return bvalue(p); }
INTERFACE INLINE int blob_size(pointer p)	{ return blength(p); }
...
static pointer mk_empty_blob(scheme *sc, int len, char fill);
static char *blob_store(scheme *sc, int len, const void *b, char fill);
...
static char *store_blob(scheme *sc, int len_blob, const char *b, char fill) {
	char *q;

	q=(char*)sc->malloc(len_blob);
	if(q==0) {
		sc->no_memory=1;
		return sc->strbuff;
	}
...
INTERFACE pointer mk_blob(scheme *sc, const char *b, int len) {
	pointer x = get_cell(sc, sc->NIL, sc->NIL);

	bvalue(x) = store_blob(sc, len, b, 0);
...
static pointer opexe_2(scheme *sc, enum scheme_opcodes op) {
     ...
     case OP_VECSET: {   /* vector-set! */
	  ...
	  case OP_BLOBLEN: { /*blob-len */
			 s_return(sc,mk_integer(sc,blength(car(sc->args))));
	  } 
